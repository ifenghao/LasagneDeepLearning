...
def norm2d(X, mean=None, var=None, regularization=1e-5):
    # 每个样本对自己均值方差归一化
    if mean is None and var is None:
        mean = np.mean(X, axis=0, keepdims=True)
        var = np.var(X, axis=0, keepdims=True)
        return (X - mean) / np.sqrt(var + regularization), mean, var
    else:
...
class ELMAELayer(Layer):
    ...
    def _get_beta(self, oneChannel):
        ...
        # 在patches上归一化
        patches, mean, var = norm2d(patches)
        # 在原图上加噪
    ...
    def get_test_output_for(self, inputX):
        ...
        for ch in xrange(channels):
            oneChannel = inputX[:, ch, :, :].reshape((batches, 1, rows, cols))
            patches = im2col(oneChannel, self.filter_size, pad=self.filter_size // 2, stride=self.stride)
            patches = norm2d(patches, self.means[ch], self.vars[ch])
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAELayer(C=self.C, n_hidden=32, filter_size=7, stride=1, noise_level=0.25)
        net['bn1'] = BNLayer()
        # net['zca1'] = ZCAWhitenLayer()
        net['pool1'] = PoolLayer(pool_size=(2, 2))
        # layer2
        net['layer2'] = ELMAELayer(C=self.C, n_hidden=64, filter_size=7, stride=1, noise_level=0.25)
        net['bn2'] = BNLayer()
