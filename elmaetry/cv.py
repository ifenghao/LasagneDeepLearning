...
1 分类器ELM重复操作，使性能稳定
2 每一个C的选择采用多个，取最优
3 计算beta不用随机通道，而使用自编码器易于评估性能，或者加噪自编码器
'''
import gc
import numpy as np
import theano
...
def compute_beta_val(Hmat, Tmat, n_iter):
    ...
    for C in cv:
        ...
        cost = np.mean((np.dot(Hmat, beta) - Tmat) ** 2)
        if cost < optcost:
            optcost = cost
            optbeta = beta
    return optbeta
...
class ELMAELayer(Layer):
    ...
    def _get_beta(self, oneChannel, addpad):
        ...
        # 计算beta
        beta = compute_beta_val(hiddens, patches, 10)
...
class Classifier(Layer):
    ...
    def get_train_output_for(self, inputX, inputy=None):
        ...
        H = relu(H)
        self.beta = compute_beta_val(H, inputy, 10)
