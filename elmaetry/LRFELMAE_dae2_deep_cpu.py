...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAELayer(C=self.C, n_hidden=56, filter_size=9, stride=1, noise_level=0.25)
        net['bn1'] = BNLayer()
        # net['zca1'] = ZCAWhitenLayer()
        # net['pool1'] = PoolLayer(pool_size=(2, 2))
        net['fmp1'] = FPLayer(pool_ratio=1.189)
        # layer2
        net['layer2'] = ELMAELayer(C=self.C, n_hidden=64, filter_size=9, stride=1, noise_level=0.25)
        net['bn2'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        # net['pool2'] = PoolLayer(pool_size=(2, 2))
        net['fmp2'] = FPLayer(pool_ratio=1.189)
        # layer3
        net['layer3'] = ELMAELayer(C=self.C, n_hidden=72, filter_size=7, stride=1, noise_level=0.25)
        net['bn3'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        # net['pool3'] = PoolLayer(pool_size=(2, 2))
        net['fmp3'] = FPLayer(pool_ratio=1.189)
        # layer4
        net['layer4'] = ELMAELayer(C=self.C, n_hidden=72, filter_size=7, stride=1, noise_level=0.25)
        net['bn4'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        # net['pool3'] = PoolLayer(pool_size=(2, 2))
        net['fmp4'] = FPLayer(pool_ratio=1.189)
        # layer5
        net['layer5'] = ELMAELayer(C=self.C, n_hidden=96, filter_size=5, stride=1, noise_level=0.25)
        net['bn5'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        # net['pool3'] = PoolLayer(pool_size=(2, 2))
        net['fmp5'] = FPLayer(pool_ratio=1.189)
        # layer6
        net['layer6'] = ELMAELayer(C=self.C, n_hidden=96, filter_size=5, stride=1, noise_level=0.25)
        net['bn6'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        # net['pool3'] = PoolLayer(pool_size=(2, 2))
        net['fmp6'] = FPLayer(pool_ratio=1.189)
        # net['cccp'] = ELMAELayer(C=self.C, n_hidden=32, filter_size=1, stride=1, noise_level=0.25)  # 降维
        return net
    ...
    def train(self, inputX, inputy):
        ...
        # print netout.shape
        # netout = self.pca.fit_transform(netout)
        # print netout.shape
    ...
    def test(self, inputX, inputy):
        ...
        netout = netout.reshape((netout.shape[0], -1))
        # netout = self.pca.transform(netout)
