...
一个ELMAE层共享一个filters和bias,投影到相同的空间,得到的特征图相加才有意义
'''
...
def normal_random_w(input_unit, output_unit):
    std = np.sqrt(2.0 / input_unit)
    return np.random.normal(loc=0, scale=std, size=(input_unit, output_unit))


def normal_random_b(input_unit, output_unit):
    std = np.sqrt(2.0 / input_unit)
    return np.random.normal(loc=0, scale=std, size=output_unit)
...
class ELMAELayer(Layer):
    ...
    def _get_beta(self, oneChannel, weight, bias, min_patch=20):
        ...
        hiddens = np.dot(noisePatch, weight) + bias
    ...
    def get_train_output_for(self, inputX):
        ...
        weight = []
        for _ in xrange(self.n_hidden):
            w = normal_random_w(input_unit=self.filter_size ** 2, output_unit=channels)
            weight.append(orthonormalize(w))
        weight = np.array(weight).transpose((2, 1, 0))
        bias = []
        for _ in xrange(channels):
            b = normal_random_b(input_unit=self.filter_size ** 2, output_unit=self.n_hidden)
            bias.append(orthonormalize(b))
        bias = np.array(bias)
        self.filters = []
        output = 0.
        for ch in xrange(channels):
            oneChannel = inputX[:, [ch], :, :]
            beta = self._get_beta(oneChannel, weight[ch], bias[ch])
...
class FPLayer(Layer):
    def __init__(self, pool_ratio, mode='max'):
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAELayer(C=self.C, n_hidden=56, filter_size=7, pad=3, stride=1,
                                   pad_=None, stride_=4, noise_level=0.25)
        net['bn1'] = BNLayer()
        # net['zca1'] = ZCAWhitenLayer()
        # net['pool1'] = PoolLayer(pool_size=(2, 2), mode='max')
        net['fmp1'] = FPLayer(pool_ratio=1.414, mode='max')
        # layer2
        net['layer2'] = ELMAELayer(C=self.C, n_hidden=60, filter_size=7, pad=3, stride=1,
                                   pad_=None, stride_=4, noise_level=0.25)
        net['bn2'] = BNLayer()
        ...
        # layer3
        net['layer3'] = ELMAELayer(C=self.C, n_hidden=64, filter_size=7, pad=3, stride=1,
                                   pad_=None, stride_=4, noise_level=0.25)
        net['bn3'] = BNLayer()
        # net['zca3'] = ZCAWhitenLayer()
        # net['pool3'] = PoolLayer(pool_size=(2, 2), mode='max')
        net['fmp3'] = FPLayer(pool_ratio=1.414, mode='max')
        # layer4
