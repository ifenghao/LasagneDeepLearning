...
class ELMAELayer(Layer):
    ...
    def _get_beta(self, oneChannel, min_patch=20):
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAELayer(C=self.C, n_hidden=12, filter_size=7, pad=3, stride=1,
        ...
        # layer2
        net['layer2'] = ELMAELayer(C=self.C, n_hidden=12, filter_size=7, pad=3, stride=1,
                                   pad_=None, stride_=2, noise_level=0.25)
        net['bn2'] = BNLayer()
        net['cccp2'] = CCCPLayer(C=self.C, n_out=72, noise_level=0.25)
        net['bn3'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        net['pool2'] = PoolLayer(pool_size=(3, 3), mode='average_exc_pad')
        # net['fmp2'] = FPLayer(pool_ratio=1.414, mode='max')
        # layer3
        # net['layer3'] = ELMAELayer(C=self.C, n_hidden=40, filter_size=7, pad=3, stride=1,
        #                            pad_=None, stride_=4, noise_level=0.25)
        # net['cccp3'] = CCCPLayer(C=self.C, n_out=40, noise_level=0.25)
        # net['bn3'] = BNLayer()
