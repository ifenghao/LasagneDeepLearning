...
class MergeLayer(Layer):
    ...
    def get_train_output_for(self, inputX):
        ...
        for name, layer in self.subnet.iteritems():
            out = layer.get_train_output(inputX)
    ...
    def get_test_output_for(self, inputX):
        output = []
        for name, layer in self.subnet.iteritems():
            out = layer.get_test_output(inputX)
            output.append(np.copy(out))
        return np.concatenate(output, axis=1)
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        net['layer1'] = ELMAELayer(C=self.C, hidden_unit=16, filter_size=7, stride=1)
        net['bn1'] = BNLayer()
        net['pool1'] = PoolLayer(pool_size=(2, 2))
        net['layer2'] = ELMAELayer(C=self.C, hidden_unit=32, filter_size=5, stride=1)
        net['bn2'] = BNLayer()
        net['layer3'] = ELMAELayer(C=self.C, hidden_unit=32, filter_size=5, stride=1)
        net['bn3'] = BNLayer()
        net['pool2'] = PoolLayer(pool_size=(2, 2))
        net['layer4'] = ELMAELayer(C=self.C, hidden_unit=64, filter_size=3, stride=1)
        net['bn4'] = BNLayer()
        net['layer5'] = ELMAELayer(C=self.C, hidden_unit=64, filter_size=3, stride=1)
        net['bn5'] = BNLayer()
