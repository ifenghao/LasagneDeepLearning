...
class LRFELMAE(object):
    ...
    def _get_train_output(self, net, inputX):
        ...
        for ch in xrange(inputX.shape[1]):
            out = np.expand_dims(inputX[:, ch, :, :], axis=1)
    ...
    def train(self, inputX, inputy):
        ...
        netout_list = self._get_train_output(self.net, inputX)
        predout_accum = 0.
        for netout in netout_list:
            netout = netout.reshape((netout.shape[0], -1))
            classifier = Classifier_cv(n_rep=2, C_range=[1., ], times_range=[7, ])
            predout_accum += classifier.train_cv(netout, inputy)
            self.classifier_list.append(deepcopy(classifier))
        predout = predout_accum / len(netout_list)
        return accuracy(predout, inputy)

    def test(self, inputX, inputy):
        netout_list = self._get_test_output(self.net, inputX)
        predout_accum = 0.
        for i, netout in enumerate(netout_list):
            netout = netout.reshape((netout.shape[0], -1))
            predout_accum += self.classifier_list[i].test_cv(netout, inputy)
        predout = predout_accum / len(netout_list)
        return accuracy(predout, inputy)
