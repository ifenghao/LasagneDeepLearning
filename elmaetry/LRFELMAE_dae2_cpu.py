...
class ELMAELayer(Layer):
    def __init__(self, C, n_hidden, filter_size, stride, stride_, noise_level):
        self.C = C
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAELayer(C=self.C, n_hidden=56, filter_size=7, stride=1, stride_=4, noise_level=0.25)
        net['bn1'] = BNLayer()
        # net['zca1'] = ZCAWhitenLayer()
        net['pool1'] = PoolLayer(pool_size=(2, 2), mode='average_exc_pad')
        # net['fmp1'] = FPLayer(pool_ratio=1.414)
        # layer2
        net['layer2'] = ELMAELayer(C=self.C, n_hidden=60, filter_size=6, stride=1, stride_=2, noise_level=0.25)
        net['bn2'] = BNLayer()
        # net['zca2'] = ZCAWhitenLayer()
        net['pool2'] = PoolLayer(pool_size=(2, 2), mode='average_exc_pad')
        # net['fmp2'] = FPLayer(pool_ratio=1.414)
        # layer3
        net['layer3'] = ELMAELayer(C=self.C, n_hidden=64, filter_size=5, stride=1, stride_=1, noise_level=0.25)
        net['bn3'] = BNLayer()
...
def main():
    ...
    tr_X, te_X, tr_y, te_y = myUtils.load.mnist_vary('rotation', onehot=True)
