...
特征图不采用相加的方式,而是利用CCCP层进行组合
由于中间会产生大量特征图,在网络加大后可以先池化后组合
'''
...
def decomp(oneChannel, beta, layer):
    ...
    if layer.pool_type == 'pool':
        pool_op = pool_decomp
    elif layer.pool_type == 'fp':
        pool_op = fp_cpu
    else:
        raise NameError
...
class ELMAEPoolLayer(Layer):
    ...
    def _get_beta(self, oneChannel, min_patch=30):
    ...
    def get_train_output_for(self, inputX):
        ...
        for ch in xrange(channels):
            ...
            output.append(decomp(oneChannel, beta, self))
    ...
    def get_test_output_for(self, inputX):
        ...
        for ch in xrange(channels):
            oneChannel = inputX[:, [ch], :, :]
            output.append(decomp(oneChannel, self.filters[ch], self))
            print 'channel', ch
        output = np.concatenate(output, axis=1)
        output = relu(output)
        return output
...
class CCCPLayer(Layer):
    ...
    def _get_beta(self, inputX):
        ...
        self.W, self.b = normal_random(input_unit=inputX.shape[1], hidden_unit=self.n_out)
...
class RandomSampleLayer(Layer):
    def __init__(self, n_out):
        self.n_out = n_out

    def get_train_output_for(self, inputX):
        batches, n_in, rows, cols = inputX.shape
        self.select_idx = np.random.permutation(n_in)[:self.n_out]
        inputX = inputX[:, self.select_idx, :, :]
        gc.collect()
        return inputX
...
class MaxSampleLayer(Layer):
    def __init__(self, n_out):
        self.n_out = n_out

    def get_train_output_for(self, inputX):
        actmean = np.mean(inputX, axis=(0, 2, 3))
        self.select_idx = np.argsort(actmean)[-self.n_out:]
        inputX = inputX[:, self.select_idx, :, :]
        gc.collect()
        return inputX
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAEPoolLayer(C=self.C, n_hidden=32, filter_size=7, pad=3, stride=1, pad_=None, stride_=4,
                                       noise_level=0.2, pool_type='pool', pool_size=(2, 2), mode='max')
        # net['bn1'] = BNLayer()
        # net['cccp1'] = CCCPLayer(C=self.C, n_out=32, noise_level=0.2)
        net['bn2'] = BNLayer()
        # layer2
        net['layer2'] = ELMAEPoolLayer(C=self.C, n_hidden=32, filter_size=7, pad=3, stride=1, pad_=None, stride_=2,
                                       noise_level=0.2, pool_type='pool', pool_size=(3, 3), mode='average_exc_pad')
        # net['bn3'] = BNLayer()
        # net['cccp2'] = CCCPLayer(C=self.C, n_out=32, noise_level=0.2)
        net['maxsample'] = MaxSampleLayer(n_out=128)
        net['bn4'] = BNLayer()
        # layer3
        # net['layer3'] = ELMAEPoolLayer(C=self.C, n_hidden=196, filter_size=7, pad=3, stride=1, pad_=None, stride_=4,
        #                                noise_level=0.2, pool_type='pool', pool_size=1.414, mode='avg')
        # net['bn3'] = BNLayer()
...
def main():
    ...
    tr_X, avg, var = myUtils.pre.norm4d(tr_X)
