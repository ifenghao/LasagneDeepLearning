...
使用全部通道并归一化,第一层可以使用较大的卷积核,剩余层都是小卷积核
层间不需要归一化,对提取的patch进行归一化,但是不用白化
'''
...
class ELMAECrossLayer(Layer):
    ...
    def _get_beta(self, inputX):
        ...
        # 生成随机正交滤波器
        filters, bias = normal_random(input_unit=channels * self.filter_size ** 2, hidden_unit=self.n_hidden)
        ...
        patches = [im2col(np.expand_dims(inputX[:, ch, :, :], axis=1), self.filter_size, stride=stride, pad=pad)
        ...
        # noisePatch = [im2col(np.expand_dims(noiseX[:, ch, :, :], axis=1), self.filter_size, stride=stride, pad=pad)
        #               for ch in xrange(channels)]
        # noisePatch = np.concatenate(noisePatch, axis=1)
        # 在patches上加噪
    ...
    def get_train_output_for(self, inputX):
        ...
        patches = [im2col(np.expand_dims(inputX[:, ch, :, :], axis=1), self.filter_size, pad=self.filter_size // 2,
                          stride=self.stride) for ch in xrange(channels)]
        patches = np.concatenate(patches, axis=1)
        patches = norm2d(patches)
        output = np.dot(patches, self.beta)
        del patches
        gc.collect()
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAECrossLayer(C=self.C, n_hidden=32, filter_size=6, stride=1, noise_level=0.25)
        # net['pool1'] = PoolLayer(pool_size=(2, 2))
        net['fmp1'] = FMPLayer(pool_ratio=1.414)
        # net['bn1'] = BNLayer()
        # layer2
        net['layer2'] = ELMAECrossLayer(C=self.C, n_hidden=46, filter_size=3, stride=1, noise_level=0.25)
        # net['pool2'] = PoolLayer(pool_size=(2, 2))
        net['fmp2'] = FMPLayer(pool_ratio=1.414)
        # net['bn2'] = BNLayer()
        # layer3
        net['layer3'] = ELMAECrossLayer(C=self.C, n_hidden=64, filter_size=3, stride=1, noise_level=0.25)
        # net['pool3'] = PoolLayer(pool_size=(2, 2))
        net['fmp3'] = FMPLayer(pool_ratio=1.414)
        # net['bn3'] = BNLayer()
        # layer4
        net['layer4'] = ELMAECrossLayer(C=self.C, n_hidden=72, filter_size=3, stride=1, noise_level=0.25)
        # net['pool3'] = PoolLayer(pool_size=(2, 2))
