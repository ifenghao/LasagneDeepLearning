...
def decomp(oneChannel, beta, layer, output):
    ...
    for start, end in zip(startRange, endRange):
        ...
        patches = pool_decomp(patches, layer.pool_size, mode=layer.mode)
...
class ELMAEPoolLayer(Layer):
    def __init__(self, C, n_hidden, filter_size, pad, stride, pad_, stride_, noise_level, pool_size, mode):
        self.C = C
    ...
    def get_train_output_for(self, inputX):
        ...
        for ch in xrange(channels):
            ...
            output = decomp(oneChannel, beta, self, output)
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAEPoolLayer(C=self.C, n_hidden=72, filter_size=7, pad=3, stride=1,
                                       pad_=None, stride_=4, noise_level=0.2, pool_size=(2, 2), mode='average_exc_pad')
        net['bn1'] = BNLayer()
        # layer2
        net['layer2'] = ELMAEPoolLayer(C=self.C, n_hidden=96, filter_size=7, pad=3, stride=1,
                                       pad_=None, stride_=2, noise_level=0.2, pool_size=(2, 2), mode='average_exc_pad')
        net['bn2'] = BNLayer()
        # layer3
        net['layer3'] = ELMAEPoolLayer(C=self.C, n_hidden=128, filter_size=7, pad=3, stride=1,
                                       pad_=None, stride_=1, noise_level=0.2, pool_size=(4, 4), mode='average_exc_pad')
        net['bn3'] = BNLayer()
        # # layer4
...
def main():
    ...
    tr_X, te_X, tr_y, te_y = myUtils.load.caltech(onehot=True)
