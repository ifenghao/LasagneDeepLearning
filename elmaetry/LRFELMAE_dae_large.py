...
def dot_pool(x, y, pool_size, ignore_border=False, stride=None, pad=(0, 0), mode='max'):
    ...
    result = []
    for start, end in zip(startRange, endRange):
        xtmp = np.dot(x[start:end], y)
        xtmp = xtmp.transpose((0, 3, 1, 2))
        result.append(pool(xtmp))
    return np.concatenate(result, axis=0)


def dot_fmp(x, y, pool_ratio):
    assert x.ndim == 4 and y.ndim == 2
    size = x.shape[0]
    batchSize = 10000
    startRange = range(0, size - batchSize + 1, batchSize)
    ...
    for start, end in zip(startRange, endRange):
        xtmp = np.dot(x[start:end], y)
        xtmp = xtmp.transpose((0, 3, 1, 2))
        result.append(fp_cpu(xtmp, pool_ratio))
    return np.concatenate(result, axis=0)


class ELMAEPoolLayer(Layer):
    def __init__(self, C, n_hidden, filter_size, stride, stride_, noise_level, pool_size, mode):
        self.C = C
    ...
    def _get_beta(self, oneChannel, min_patch=10):
        ...
        # 卷积前向输出,和取patch时一致
        patches = im2col(oneChannel, self.filter_size, stride=self.stride_, pad=None)
        ...
        # noisePatch = im2col(noiseChannel, self.filter_size, stride=self.stride_, pad=None)
        # 在patches上加噪
        ...
        hiddens = np.dot(patches, filters) + bias
    ...
    def get_train_output_for(self, inputX):
        ...
        for ch in xrange(channels):
            ...
            patches = norm2d(patches)
            patches = whiten(patches, self.P)
            patches = patches.reshape((batches, orows, ocols, -1))
            output += dot_pool(patches, beta, self.pool_size, mode=self.mode)
    ...
    def get_test_output_for(self, inputX):
        ...
        for ch in xrange(channels):
            ...
            patches = whiten(patches, self.P)
            patches = patches.reshape((batches, orows, ocols, -1))
            output += dot_pool(patches, self.filters[ch], self.pool_size, mode=self.mode)
        output = relu(output)
        return output
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAEPoolLayer(C=self.C, n_hidden=256, filter_size=9, stride=1,
                                       stride_=4, noise_level=0.25, pool_size=(2, 2), mode='max')
        # net['bn1'] = BNLayer()
        # layer2
        net['layer2'] = ELMAEPoolLayer(C=self.C, n_hidden=324, filter_size=9, stride=1,
                                       stride_=1, noise_level=0.25, pool_size=(8, 8), mode='average_exc_pad')
        net['bn2'] = BNLayer()
        # layer3
...
class Ensemble(object):
    ...
    def train(self, inputX, inputy):
        ...
        for i in xrange(self.n_estimator):
            ...
            print 'model', i, acc
            gc.collect()
        # row_idx = np.arange(len(inputX))
        # voter = np.zeros_like(inputy, dtype=np.int)
        # for col_idx in predout_list:
        #     voter[row_idx, col_idx] += 1
        # return accuracy(voter, inputy)
