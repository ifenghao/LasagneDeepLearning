...
def decomp(oneChannel, beta, layer):
    ...
    for start, end in zip(startRange, endRange):
        ...
        if layer.n_out is not None:
            patches = patches[:, layer.select_idx, :, :]
...
class ELMAEPoolLayer(Layer):
    def __init__(self, C, n_hidden, filter_size, pad, stride, pad_, stride_, noise_level,
                 pool_type, pool_size, mode, n_out):
        self.C = C
    ...
    def get_train_output_for(self, inputX):
        ...
        if self.n_out is not None:
            self.select_idx = np.random.permutation(channels)[:self.n_out]
        self.filters = []
        output = []
...
class LRFELMAE(object):
    ...
    def _build(self):
        net = OrderedDict()
        # layer1
        net['layer1'] = ELMAEPoolLayer(C=self.C, n_hidden=56, filter_size=7, pad=3, stride=1, pad_=None, stride_=4,
        ...
        net['bn2'] = BNLayer()
        # layer2
        net['layer2'] = ELMAEPoolLayer(C=self.C, n_hidden=60, filter_size=7, pad=3, stride=1, pad_=None, stride_=4,
                                       noise_level=0.2, pool_type='fp', pool_size=1.414, mode='max',
                                       n_out=2)
        # net['bn3'] = BNLayer()
        ...
        net['bn4'] = BNLayer()
        # layer3
        net['layer3'] = ELMAEPoolLayer(C=self.C, n_hidden=64, filter_size=7, pad=3, stride=1, pad_=None, stride_=4,
                                       noise_level=0.2, pool_type='fp', pool_size=1.414, mode='max',
                                       n_out=2)
        net['bn3'] = BNLayer()
